...
var readPublicResourceSchema = joi.object({
...
BBT.Connector.prototype.readPublic = function (params, callback) {
  var self = this
  var valid = readPublicResourceSchema.validate(params)

  if (valid.error) {
    return callback(extend({}, valid.error, {code: 11}))
  } else {
...
BBT.Connector.prototype.readPublicResource = function (params, callback) {
  return this.readPublic(params, callback)
}

var readResourceSchema = joi.object({
  channel: joi.string().regex(/^\w+$/).min(2).max(64).required(),
  resource: joi.string().regex(/^\w+$/).min(2).max(64).required(),
  limit: joi.number().integer().min(1).max(50000).optional(),
  source: joi.string().optional().valid('raw', 'hour-stats', 'day-stats'),
...
BBT.Connector.prototype.read = function (params, callback) {
  var self = this
  var valid = readResourceSchema.validate(params)
...
BBT.Connector.prototype.write = function (params, callback) {
  var self = this
  var valid = writeResourceSchema.validate(params)
...
var writeBulkSchema = joi.object({
  channel: joi.string().regex(/^\w+$/).min(2).max(64).required(),
...
BBT.Connector.prototype.writeBulk = function (params, callback) {
  var self = this
  var valid = writeBulkSchema.validate(params)
...
BBT.Connector.prototype.delete = function(params, callback) {
  var self = this
  var valid = deleteRecordSchema.validate(params)
...
BBT.Connector.prototype.update = function(params, callback) {
  var self = this
  var valid = updateResourceSchema.validate(params)
...
BBT.Connector.prototype.publish = function (params, callback) {
  var self = this
  var valid = publishSchema.validate(params)
...
var publishBulkSchema = joi.object({
  channel: joi.string().regex(/^(private-)?\w+$/).min(2).max(64).required(),
  records: joi.array().items(joi.object({
    resource: joi.string().regex(/^\w+$/).min(2).max(64).required(),
    data: joi.any().required()
...
BBT.Connector.prototype.publishBulk = function (params, callback) {
  var self = this
  var valid = publishBulkSchema.validate(params)
...
var getUserConnectionSchema = joi.object({
...
BBT.Connector.prototype.getUserConnections = function(params, callback) {
  ...
  if (self.authType === 'token') {
    ...
    var valid = getUserConnectionSchema.validate(params)
...
var dropUserConnectionSchema = joi.object({
...
BBT.Connector.prototype.dropUserConnection = function(params, callback) {
  ...
  var valid = dropUserConnectionSchema.validate(params)
...
var validvtype = joi.string().required()

var addResourceSchema = joi.object({
  channel: joi.string().regex(/^\w+$/).min(2).max(64).required(),
  resource: joi.object({
    name: joi.string().regex(/^\w+$/).min(2).max(64).required(),
    label: joi.string().max(64).optional(),
    description: joi.string().optional(),
    vtype: validvtype.valid.apply(validvtype, BBT.AttributeTypesLabels),
...
BBT.Connector.prototype.addResource = function (params, callback) {
  ...
  var valid = addResourceSchema.validate(params)
...
var getResourceSchema = joi.object({
  channel: joi.string().regex(/^\w+$/).min(2).max(64).required(),
...
BBT.Connector.prototype.getResource = function (params, callback) {
  ...
  if (self.authType === 'token') {
    ...
    var valid = getResourceSchema.validate(params)
...
var deleteResourceSchema = joi.object({
  channel: joi.string().regex(/^\w+$/).min(2).max(64).required(),
  resource: joi.string().regex(/^\w+$/).min(2).max(64).required(),
})
...
BBT.Connector.prototype.deleteResource = function (params, callback) {
  ...
  var valid = deleteResourceSchema.validate(params)
...
var addChannelSchema = joi.object({
  name: joi.string().regex(/^\w+$/).min(2).max(64).required(),
  label: joi.string().max(64).optional(),
  description: joi.string().optional(),
...
BBT.Connector.prototype.addChannel = function (params, callback) {
  ...
  var valid = addChannelSchema.validate(params)
...
var getChannelSchema = joi.object({
  channel: joi.string().regex(/^\w+$/).min(2).max(64).required(),
})
...
BBT.Connector.prototype.getChannel = function (channel, callback) {
  ...
  if (self.authType === 'token') {
    ...
    var valid = getChannelSchema.validate({channel: channel})
    if (valid.error) {
      return callback(extend({}, valid.error, {code: 11}))
    }
...
var deleteChannelSchema = joi.object({
  channel: joi.string().regex(/^\w+$/).min(2).max(64).required(),
})
...
BBT.Connector.prototype.deleteChannel = function (channel, callback) {
  ...
  var valid = deleteChannelSchema.validate({channel: channel})
...
var regenerateChannelTokenSchema = joi.object({
  channel: joi.string().regex(/^\w+$/).min(2).max(64).required(),
})
...
BBT.Connector.prototype.regenerateChannelToken = function (channel, callback) {
  ...
  var valid = regenerateChannelTokenSchema.validate({channel: channel})

  if (valid.error) {
    return callback(extend({}, valid.error, {code: 11}))
  } else {
    return self.getData('/v1/channels/' + channel + '/token/regenerate', null, callback)
  }
}
...
var getBeerulesQuerySchema = joi.object({
  type: joi.string().valid.apply(joi.string(), BBT.ActionTypes).optional(),
  event: joi.string().valid.apply(joi.string(), BBT.TriggerTypes).optional(),
...
BBT.Connector.prototype.getBeerules = function(query, callback) {
  ...
  if (query) {
    var valid = getBeerulesQuerySchema.validate(query)

    if (valid.error) {
      return callback(extend({}, valid.error, {code: 11}))
    } else {
      return self.getData('/v1/beerules', query, callback)
    }
  } else {
...
var beeruleInvocationSchema = joi.object({
  channel: joi.string().regex(/^(private-)?\w+$/).min(2).max(64).required(),
  resource: joi.string().regex(/^\w+$/).min(2).max(64).required(),
  ispublic: joi.boolean().optional(),
...
BBT.Connector.prototype.invokeBeerule = function (beeruleid, body, callback) {
  ...
  var valid = beeruleInvocationSchema.validate(body)
...
var genericBeeruleSchemaTemplate = {
  name: joi.string().min(2).max(48).required(),
  description: joi.string().optional(),
  owner: joi.string().optional(),
  trigger: joi.object({
    event: joi.string().valid.apply(joi.string(), BBT.TriggerTypes).required(),
    channel: joi.string().regex(/^(\*|(private-)?(\w\w+))$/).max(64).required(),
    resource: joi.string().regex(/^(\*|\w\w+)$/).max(64).required()
  }).required(),
  condition: joi.string().optional(),
  action: {
    type: joi.string().valid.apply(joi.string(), BBT.ActionTypes).required(),
  }
}

var genericBeeruleSchema = joi.object(genericBeeruleSchemaTemplate)

var publishBeeruleSchema = joi.object(extend({}, genericBeeruleSchemaTemplate, {
  action: joi.object({
    type: 'publish',
    channel: joi.string().regex(/^(private-)?\w+$/).min(2).max(64).required(),
    resource: joi.string().regex(/^\w+$/).min(2).max(64).required(),
...
var writeBeeruleSchema = joi.object(extend({}, genericBeeruleSchemaTemplate, {
  action: joi.object({
    type: 'write',
    channel: joi.string().regex(/^\w+$/).min(2).max(64).required(),
    resource: joi.string().regex(/^\w+$/).min(2).max(64).required(),
...
var smsBeeruleSchema = joi.object(extend({}, genericBeeruleSchemaTemplate, {
  action: joi.object({
    type: 'sms',
...
var emailBeeruleSchema = joi.object(extend({}, genericBeeruleSchemaTemplate, {
  action: joi.object({
    type: 'email',
...
var webhookBeeruleSchema = joi.object(extend({}, genericBeeruleSchemaTemplate, {
  action: joi.object({
    type: 'webhook',
...
var fcmBeeruleSchema = joi.object(extend({}, genericBeeruleSchemaTemplate, {
  action: joi.object({
    type: 'fcm',
...
BBT.Connector.prototype.createBeerule = function (params, callback) {
  ...
  var valid = schemaToCheck.validate(params)
...
var iamTokenSchema = joi.object({
  name: joi.string().min(2).max(48).required(),
  description: joi.string().optional(),
  acl: joi.array().items(joi.object({
    action: joi.string().valid.apply(joi.string(), BBT.AclTypes).required(),
    resource: joi.array()
    .when(
      'action',
      {
        is: joi.string().valid.apply(joi.string(), BBT.AclDataTypes),
...
BBT.Connector.prototype.createIAMToken = function (params, callback) {
  ...
  var valid = iamTokenSchema.validate(params)
...
var iamTokenUpdateSchema = joi.array().items(joi.object({
  action: joi.string().valid.apply(joi.string(), BBT.AclTypes).required(),
  resource: joi.array()
  .when(
    'action',
...
BBT.Connector.prototype.updateIAMToken = function (iamtokenid, acl, callback) {
  ...
  var valid = iamTokenUpdateSchema.validate(acl)

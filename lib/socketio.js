...
Connection.prototype.do_subscribe = function(subscription) {
  ...
  if( authNeeded() ) {
    if( ! self.bbt.transport.auth_endpoint ) return self.bbt.emit('subscribeError', 'Authentication error: Missing authentication endpoint!', self);
    if(self.connected && self.connection && self.connection.io.engine.id && self.connection.io.engine.id) {
      ...
      if(self.bbt.transport.auth_method === 'get') {
        ...
        request(options, function (error, response, body) {
            if(error) return self.bbt.emit('subscribeError', error, self);
            if (!error && response.statusCode == 200) {
              if( typeof body === 'string' ) {
                try {
                  body = JSON.parse(body);
                } catch(e) {
                  return self.bbt.emit('subscribeError', 'Unable to authenticate client', self);
                }
              }
              args.sig = body.auth;
              ...
              if(self.send('control', 'subscribe', args)) {
                ...
                return self.bbt.emit('subscribed', self);
              }else {
                  return self.bbt.emit('subscribeError', 'Unexpected error encountered while subscribing', self);
              }
            }else {
                return self.bbt.emit('subscribeError', 'Unable to authenticate client', self);
            }
        ...
        request(options, function (error, response, body) {
            if(error) return self.bbt.emit('subscribeError', 'Unable to authenticate client', self);
            if (!error && response.statusCode == 200) {
              if( typeof body === 'string' ) {
                try {
                  body = JSON.parse(body);
        ...
        if( !sig ) return self.bbt.emit('subscribeError', 'Unable to authenticate client', self);
        args.sig = sig.auth;
        ...
        return self.bbt.emit('subscribeError', 'Unsupported authentication method!', self);
      }
    } else {
      return self.bbt.emit('subscribeError', 'Connection error encountered', self);
    }
  }else {
    if(self.send('control', 'subscribe', args)) {
      subscription.subscribe();
...
Connection.prototype.unsubscribe = function(args) {
  ...
  if(subscription) {
    ...
    if( this.send('control', 'unsubscribe', {channel: args.channel, resource: args.resource }) ) {
      ...
      return self.bbt.emit('unsubscribeError', {channel: args.channel, resource: args.resource });
    }
  }
  return;
}
...
Connection.prototype.publish = function(args) {
  ...
  if(subscription && subscription.hasWritePermission()) {
    if(this.send('stream', 'emit', {channel: args.channel, resource: args.resource, data: args.data})) {
      ...
      return self.bbt.emit('publishError', 'Error while publishing message', args);
    }
  }
...
Connection.prototype.write = function(args) {
  ...
  if(subscription && subscription.hasWritePermission()) {
    if(this.send('stream', 'write', {channel: args.channel, resource: args.resource, data: args.data})) {
      ...
      return self.bbt.emit('writeError', 'Error while sending write message', args);
    }
  }
  return self.bbt.emit('writeError', 'Permission error: cant\'t write on the given resource!', args);
}

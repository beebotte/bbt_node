...
Connection.prototype.do_subscribe = function(subscription) {
  ...
  if( authNeeded() ) {
    if( ! self.bbt.transport.auth_endpoint ) {
...
Connection.prototype.unsubscribe = function(args) {
  ...
  if(subscription) {
    ...
    if( this.send('control', 'unsubscribe', {channel: args.channel, resource: args.resource }) ) {
      self.bbt.emit('unsubscribed', {channel: args.channel, resource: args.resource });
    } else {
      self.bbt.emit('unsubscribeError', new Error('Unexpected error encountered while unsubscribing'), {channel: args.channel, resource: args.resource });
    }
  }
  return self.bbt;
}

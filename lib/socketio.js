...
Connection.prototype.connect = function () {
  ...
  if (self.connection) {
    ...
    self.connection = require('socket.io-client')(self.getWsUrl(), {query: query });
  }
...
Connection.prototype.subscribe = function(args, callback) {
  ...
  if(subscription) {
    if(subscription.update(args)) this.do_subscribe(subscription);
  }else {
...
Connection.prototype.do_subscribe = function(subscription) {
  var self = this;
  if(!self.connection.connected) return self.bbt;
  var args = {};
  ...
  if( authNeeded() ) {
    ...
    if(self.connection && self.connection.connected && self.connection.io.engine.id) {

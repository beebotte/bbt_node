...
BBT.Connector = function(options) {
    ...
    if (options.keyId && options.secretKey) {
        ...
        throw new Error('(BBT.Connector) Parameter Error: You must provide your key ID and your secret key!');
    }
    ...
    this.hostname = options.hostname || 'x.beebotte.com';
...
var readPublicResourceSchema = {
    owner: joi.types.String().min(2).max(30).required(),
    device: joi.types.String().min(2).max(30).required(),
    service: joi.types.String().max(30).required(),
    resource: joi.types.String().max(30).required(),
    limit: joi.types.Number().integer().min(1).max(250).optional(),
    source: joi.types.String().optional().valid('live', 'hour', 'day', 'week', 'month'),
    metric: joi.types.String().optional().valid('avg', 'min', 'max', 'count'),
};
...
var writeBulkSchema = {
    device: joi.types.String().min(2).max(30).required(),
    data: joi.types.Array().includes(joi.types.Object({
        service: joi.types.String().max(30).required(),
        resource: joi.types.String().max(30).required(),
        value: joi.types.Any().required()
    }))
...
var addResourceSchema = {
    device: joi.types.String().min(2).max(30).required(),
    service: joi.types.String().max(30).required(),
    resource: joi.types.Object({
        name: joi.types.String().min(3).max(30).required(),
        description: joi.types.String().optional(),
        vtype: joi.types.String().required().valid(BBT.AttributeTypesLabels),
    }).required()
};
/*
...
var addServiceSchema = {
    device: joi.types.String().min(2).max(30).required(),
    service: joi.types.Object({
        name: joi.types.String().max(30).required(),
        description: joi.types.String().optional(),
        resources: joi.types.Array([{
            name: joi.types.String().max(30).required(),
            description: joi.types.String().optional(),
            vtype: joi.types.String().required().valid(BBT.AttributeTypesLabels),
        }]).required(),
...
var addDeviceSchema = {
    ...
    description: joi.types.String().optional(),
    ispublic: joi.types.Boolean().optional(),
    services: joi.types.Array([{
        name: joi.types.String().max(30).required(),
        description: joi.types.String().optional(),
        resources: joi.types.Array([{
            name: joi.types.String().max(30).required(),
...
var eventSchema = {
    ...
    service: joi.types.String().max(30).required(),
    resource: joi.types.String().max(30).required(),
    data: joi.types.Any().required(),
    source: joi.types.String().max(30).optional()
}

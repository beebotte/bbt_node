...
//{channel, resource, type, data}
BBT.Connector.prototype.getUserinfo = function(params, callback) {
  var self = this;
  joi.validate(params, userInfoSchema, function(err, value) {
    if(err) return callback({error: {message: JSON.stringify(err.details), code: 11}});

    self.getData('/v1/connections/getinfo/' + params.userid, (params.sid)? {sid: params.sid}: null, callback);
  });
}
